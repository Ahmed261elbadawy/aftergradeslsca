...
if (useCloud) {
  ...
  function ensureReady() {
    if (!readyPromise) {
      ...
      `)).then(() => pool.query(`
        ALTER TABLE submissions ADD COLUMN IF NOT EXISTS over_21 TEXT
      `)).then(() => pool.query(`
  ...
  impl = {
    async insert({ name, email, phone, guestOf, socialMedia, over21, screenshotUrl, paidTo }) {
      await ensureReady();
      await pool.query(
        `INSERT INTO submissions (name, email, phone, guest_of, social_media, over_21, screenshot_url, paid_to)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
        [name, email, phone, guestOf, socialMedia, over21, screenshotUrl, paidTo]
      );
    },
    async list() {
      await ensureReady();
    ...
    async findRecentDuplicate(phone, socialMedia, windowSeconds) {
      await ensureReady();
      const { rows } = await pool.query(
        `SELECT id FROM submissions
         WHERE phone = $1 AND social_media = $2
           AND created_at > now() - ($3 || ' seconds')::interval
         ORDER BY created_at DESC LIMIT 1`,
        [phone, socialMedia, windowSeconds]
      );
      return rows[0] || null;
    },
    async recordView() {
  ...
  if (!existingCols.includes('over_21')) db.exec('ALTER TABLE submissions ADD COLUMN over_21 TEXT');
  ...
  impl = {
    async insert({ name, email, phone, guestOf, socialMedia, over21, screenshotUrl, paidTo }) {
      db.prepare(`
        INSERT INTO submissions (name, email, phone, guest_of, social_media, over_21, screenshot_url, paid_to)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(name, email, phone, guestOf, socialMedia, over21, screenshotUrl, paidTo);
    },
    async list() {
    ...
    async findRecentDuplicate(phone, socialMedia, windowSeconds) {
      const row = db.prepare(
        `SELECT id FROM submissions
         WHERE phone = ? AND social_media = ?
           AND created_at > datetime('now', '-' || ? || ' seconds')
         ORDER BY created_at DESC LIMIT 1`
      ).get(phone, socialMedia, windowSeconds);
      return row || null;
    },
    async recordView() {
